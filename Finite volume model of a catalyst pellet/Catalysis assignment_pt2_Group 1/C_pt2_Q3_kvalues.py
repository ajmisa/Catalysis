...
def main():
    ...
    k_values = [0.01, 0.1, 1, 10, 100, 1000]  # Reaction rate constants
    ...
    plt.plot(k_values, errors_L1, label='$L_1$ Error', marker='o')
    plt.plot(k_values, errors_L2, label='$L_2$ Error', marker='s')
    plt.plot(k_values, errors_Lmax, label='$L_inf$ Error', marker='^')

    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Reaction Rate Constant ($k$)')
    plt.ylabel('Error')
    plt.title('Error Metrics for Different Reaction Rate Constants ($k$)')
    plt.legend()
    plt.grid()
    plt.show()
